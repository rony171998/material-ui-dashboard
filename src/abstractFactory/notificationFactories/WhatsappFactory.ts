// notificationFactories.ts
import axios from 'axios';
import { NotificationProcessorFactory, NotificationMethod, NotificationResult, NotificationDetails, NotificationFormComponent } from './interfaces';
import { WhatsAppForm } from '../notificationForm/WhatsappForm';

// Factory para Whatsapp
export class WhastappFactory implements NotificationProcessorFactory {
  createNotificationMethod(config: any): NotificationMethod {
    return new WhastappNotification(config);
  }

  createFormComponent(): NotificationFormComponent {
    return WhatsAppForm;
  }

  getType(): string {
    return 'whatsapp';
  }
}

class WhastappNotification implements NotificationMethod {
  constructor(private whastappData: { whatsapp: string }) {}

  async processNotification(amount: number): Promise<NotificationResult> {
    try {
      const response = await axios.post(`http://localhost:8080/api/notificacion/whatsapp`, {
        ...this.whastappData
      }, {
        headers: {
          'Content-Type': 'application/json'
        }
      });

      // Axios ya parsea la respuesta JSON automáticamente
      // y encapsula los errores HTTP (status != 2xx)
      return {
        success: true,
        transactionId: `PP-${Math.random().toString(36).substring(2, 10)}`,
        message: 'Whatsapp notification successful ' + response.data
      };
    } catch (error) {
      // Manejo de errores con Axios
      let errorMessage = 'Whatsapp notification failed';
      
      if (axios.isAxiosError(error)) {
        errorMessage = error.response?.data?.message || 
                      error.message || 
                      'Error processing Whatsapp notification';
      } else if (error instanceof Error) {
        errorMessage = error.message;
      }

      return {
        success: false,
        message: errorMessage
      };
    }
  }

  getNotificationDetails(): NotificationDetails {
    return {
      type: 'Whatsapp',
      fees: 0.02,
      currency: 'USD'
    };
  }
}